    ...
    <section id="certifications" className="mx-auto max-w-6xl px-6 py-24 md:px-8">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {CERTIFICATIONS.map((c, i) => (
          <motion.div
            key={c.title}
            className="rounded-xl border border-[#233554] bg-[#112240]/50 p-6 backdrop-blur-sm transition hover:border-violet-500/30"
            initial={{ opacity: 0, y: 16 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, margin: '-60px' }}
            transition={{ duration: 0.4, delay: i * 0.08 }}
          >
            <h3 className="text-lg font-semibold text-[#ccd6f6]">{c.title}</h3>
            <p className="mt-1 font-mono text-sm text-slate-500">{c.issuer}</p>
            <div className="mt-4">
              {c.link ? (
                <a
                  href={c.link}
                ...
                <p className="inline-flex items-center gap-2 text-sm text-amber-400/90">
